...
export async function generateNodeProject(projectName, useTypeScript = false) {
...
process.on('unhandledRejection', (err, promise) => {
  ...
  const { installDeps, initGit, openEditor } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'installDeps',
  ...
  if (installDeps) {
    ...
    if (installResult.code === 0) {
      ...
      spinner.warn('Some dependencies failed to install. You can run "npm install" manually.');
    }
  }
