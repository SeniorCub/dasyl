  ...
  try {
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      ...
      if (config.autoUpdate === null && !options.silent) {
        ...
        console.log(chalk.yellow('│                                            │'));
        console.log(chalk.yellow('│') + chalk.bold('  🤖 Enable auto-updates?                   ') + chalk.yellow('│'));
        console.log(chalk.yellow('│') + `  ${chalk.cyan('y')} - Yes, auto-update when available      ` + chalk.yellow('│'));
        console.log(chalk.yellow('│') + `  ${chalk.cyan('n')} - No, just notify me                ` + chalk.yellow('│'));
        console.log(chalk.yellow('│') + `  ${chalk.cyan('u')} - Update now and ask later          ` + chalk.yellow('│'));
        console.log(chalk.yellow('╰────────────────────────────────────────────╯\n'));
        
        // Don't wait for input during normal CLI operations
        if (!process.stdin.isTTY) {
          console.log(chalk.cyan(`Run: npm install -g ${packageName}`));
          return;
        }
        
        return; // Let the user continue with their command
      }
