...
async function performAutoUpdate(packageName, newVersion) {
  return new Promise((resolve) => {
    ...
    updateProcess.on('close', (code) => {
      if (code === 0) {
        console.log(chalk.green(`[v] Successfully updated to dasyl v${newVersion}!`));
        console.log(chalk.cyan('[!] Please run your command again to use the latest version.\n'));
      } else {
        console.log(chalk.red('[x] Auto-update failed. Please update manually:'));
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      ...
      if (config.autoUpdate === null && !options.silent) {
        ...
        if (updateChoice === 'y') {
          ...
          console.log(chalk.green('[v] Auto-updates enabled!'));
...
export function handleAutoUpdateSetting(enable) {
  ...
  if (enable) {
    console.log(chalk.green('[v] Auto-updates enabled! DASYL will update automatically when new versions are available.'));
  } else {
